// GameNative Quest OpenXR runtime bridge (PE side, freestanding — no CRT).
//
// This DLL is what a Windows OpenXR game loads (via the Khronos loader and
// XR_RUNTIME_JSON / the registry) inside Wine. It forwards tracking, timing,
// input and lifecycle over the GameNative control plane (TCP 127.0.0.1:38476,
// served by XrBridgeServer.kt in the Android app).
//
// Floats on the wire are fixed-point "micro-units": value * 1e6 as a signed
// decimal integer, so this CRT-less module only ever parses integers.
//
// GPU swapchains are allocated by the Wine Unix companion and registered with
// the Quest compositor as dma-bufs. This PE side enforces OpenXR call order and
// parses projection layers; the Unix side owns Vulkan allocation and fences.
...
#include "../../cpp/third_party/openxr/openxr.h"
#include "../../cpp/third_party/openxr/openxr_loader_negotiation.h"
...
/* clang may lower aggregate copies to memcpy even in this CRT-free DLL. */
...
// Parse `key=<signed int>` where the key starts the line or follows a space,
// so short keys ("sx") never match inside longer ones ("gsx").
...
// Micro-unit fixed point: wire value is float * 1e6 as signed decimal.
...
#define GN_GENERIC_APPEND 0x0004UL /* FILE_APPEND_DATA */
...
static int gn_log_state = 0; /* 0 = unknown, 1 = enabled, -1 = disabled */
...
#else
/* OpenXR handles are pointers in the C ABI. Keep synthetic handles inside the
 * 32-bit address width instead of silently truncating the 64-bit tags. */
...
/* Input components resolvable from a Quest Touch binding path. */
enum {
    GN_COMP_NONE = 0,
    GN_COMP_PRIMARY,     /* A / X click */
    GN_COMP_SECONDARY,   /* B / Y click */
    GN_COMP_STICK_CLICK,
    GN_COMP_MENU,
    GN_COMP_TRIGGER,
    GN_COMP_SQUEEZE,
    GN_COMP_STICK,       /* vector2 */
    GN_COMP_STICK_X,
    GN_COMP_STICK_Y,
    GN_COMP_GRIP_POSE,
    GN_COMP_AIM_POSE,
...
typedef struct {
    ...
    int action_set_idx;
    XrActionType type;
    unsigned char component[2]; /* per hand, GN_COMP_* */
    unsigned char binding_priority[2];
    unsigned char active_hands; /* bit 0 left, bit 1 right; set by xrSyncActions */
    char name[64];
} GnAction;
...
typedef struct {
    int used;
    int action_idx;
    int hand; /* -1 = derive from action bindings */
    XrPosef pose_in_action_space;
} GnActionSpace;

typedef struct {
    int active;
    unsigned buttons; /* bit0 primary, bit1 secondary, bit2 stick click, bit3 menu */
    float trigger, squeeze, sx, sy;
    float grip[7]; /* qx qy qz qw px py pz */
    float aim[7];
} GnHandState;
...
/* Which graphics binding the app negotiated (for swapchain format lists). */
...
/*
 * Linux FD extensions are enabled internally by the XR-scoped winevulkan
 * module. Advertising them to a Windows Vulkan application makes winevulkan
 * reject vkCreateDevice with VK_ERROR_EXTENSION_NOT_PRESENT.
 */
static const char* GN_VULKAN_DEVICE_EXTENSIONS = "";

/* Simple spinlock: real games call OpenXR from several threads and the bridge
 * shares one socket. Contention is short (loopback round-trip). */
...
static int64_t gn_d3d_format_to_vk(int64_t format) {
    switch (format) {
        case 29: return 43; /* DXGI_FORMAT_R8G8B8A8_UNORM_SRGB */
        case 28: return 37; /* DXGI_FORMAT_R8G8B8A8_UNORM */
        case 91: return 50; /* DXGI_FORMAT_B8G8R8A8_UNORM_SRGB */
        case 87: return 44; /* DXGI_FORMAT_B8G8R8A8_UNORM */
        default: return 0;
    }
}
...
/* Raw request/response on the (already held) lock. */
static int gn_bridge_call_locked(const char* command, char* response, gn_size response_size) {
    ...
    if (gn_bridge_socket == GN_INVALID_SOCKET) {
        ...
        /* handshake with the freshly opened socket */
        {
            char hello[64];
            const char* h = "HELLO\n";
...
/* -1 when the path is neither hand. */
...
/* Map a full binding path (e.g. /user/hand/left/input/trigger/value) to a component. */
static int gn_component_from_binding(const char* str) {
    ...
    if (gn_contains(str, "/input/select")) return GN_COMP_TRIGGER; /* simple_controller */
    return GN_COMP_NONE;
}

/* Bindings for the actual Quest Touch profile must outrank generic fallback
 * profiles regardless of the order in which the application suggests them. */
...
/* First hand an action is bound on (fallback when no subaction path given). */
...
static float gn_comp_value(int hand, int comp) {
    ...
    switch (comp) {
        ...
        case GN_COMP_STICK: return h->sx; /* change detection proxy */
        default: return 0.0f;
    }
}
...
static void gn_update_pose_velocity(
    ...
    if (*previous_time > 0 && time > *previous_time) {
        ...
        if (detect_recenter &&
            ...
            /* A tracking-origin jump is not physical velocity. Re-anchor LOCAL
               at the new head pose and reset the velocity baseline. */
...
static XrResult XRAPI_CALL gn_xrEnumerateReferenceSpaces(XrSession session, gn_uint32 capacity, gn_uint32* count, XrReferenceSpaceType* spaces) {
    if (session != gn_session) return XR_ERROR_HANDLE_INVALID;
    XrResult r = gn_copy_props(3, capacity, count);
    if (XR_FAILED(r) || capacity == 0) return r;
    spaces[0] = XR_REFERENCE_SPACE_TYPE_VIEW;
    if (capacity > 1) spaces[1] = XR_REFERENCE_SPACE_TYPE_LOCAL;
    if (capacity > 2) spaces[2] = XR_REFERENCE_SPACE_TYPE_STAGE;
    return XR_SUCCESS;
}
...
static XrVector3f gn_quat_rotate(XrQuaternionf q, XrVector3f v) {
    XrVector3f out;
    /* Unit-quaternion vector rotation, expanded to avoid libm. */
...
/* Resolve a space into the Quest tracking coordinate system. */
...
static XrResult XRAPI_CALL gn_xrCreateSwapchain(XrSession session, const XrSwapchainCreateInfo* createInfo, XrSwapchain* swapchain) {
    ...
    if (gn_gfx_api == GN_GFX_D3D11) {
        ...
        desc.BindFlags = 0x8; /* D3D11_BIND_SHADER_RESOURCE */
        ...
        gn_zero_memory(&desc, sizeof(desc));
        desc.Dimension = 3; /* D3D12_RESOURCE_DIMENSION_TEXTURE2D */
        desc.Width = createInfo->width;
        desc.Height = createInfo->height;
        ...
        desc.SampleDesc.Count = createInfo->sampleCount;
        if (createInfo->usageFlags & XR_SWAPCHAIN_USAGE_COLOR_ATTACHMENT_BIT)
            desc.Flags |= 0x1; /* D3D12_RESOURCE_FLAG_ALLOW_RENDER_TARGET */
        if (createInfo->usageFlags & XR_SWAPCHAIN_USAGE_UNORDERED_ACCESS_BIT)
            desc.Flags |= 0x4; /* D3D12_RESOURCE_FLAG_ALLOW_UNORDERED_ACCESS */
        for (gn_uint32 i = 0; i < args.image_count; ++i) {
...
/* Resolve action + optional subaction to (hand, comp); returns 0 on failure. */
