...
// Fill a sockaddr_un for `path`. A leading '@' selects the Linux abstract namespace
// (no filesystem entry — required because the Android app process has no writable /tmp,
// and abstract sockets are still visible across the proot/container net namespace).
...
// Parse "key=<int>" out of a control line. Returns fallback if absent. Base 0 lets the
// producer send hex (0x...) for FourCC/modifier or decimal for sizes.
...
// Receive exactly one fd carried as SCM_RIGHTS ancillary data alongside a 1-byte payload.
// Returns the received fd (caller owns it) or -1 on failure.
...
FrameTransport::~FrameTransport() { stop(); }

void FrameTransport::start(const std::string& socketPath) {
    bool expected = false;
...
void FrameTransport::acceptLoop() {
    ...
    if (!abstract) ::unlink(socketPath_.c_str());  // stale socket from a previous run
...
void FrameTransport::serviceClient(int clientFd) {
    std::string line;
    while (running_.load() && readLine(clientFd, line)) {
        if (line.rfind("HELLO", 0) == 0) {
            ...
            if (!handleBufferLine(clientFd, line)) {
                ...
                return;  // socket framing is now suspect; drop the client
            }
        } else if (line.rfind("DMABUF", 0) == 0) {
            if (!handleDmabufLine(clientFd, line)) {
                writeAll(clientFd, "ERR dmabuf\n", 11);
                return;  // an unconsumed ancillary fd would desync framing; drop the client
            }
...
bool FrameTransport::handleBufferLine(int clientFd, const std::string& line) {
    ...
    // Ack so the producer proceeds to send the handle (mirrors GPUImage's handshake),
    // then receive the AHardwareBuffer handle directly off the socket. No GL here.
...
bool FrameTransport::handleDmabufLine(int clientFd, const std::string& line) {
    long long eye = parseKey(line, "eye", -1);
    ...
    // Ack so the producer sends one fd per plane, then receive them via SCM_RIGHTS.
    if (!replyOk(clientFd)) return false;
    for (int plane = 0; plane < frame.planeCount; ++plane) {
...
bool FrameTransport::handleFrameLine(int clientFd, const std::string& line) {
...

    {
        ...
        // If the render thread never claimed the previous presentation it was
        // superseded before sampling. Release that slot immediately so a fast
        // producer cannot deadlock after filling its buffer ring.
        ...
        latest_[eye] = registered; // Shallow snapshot; the registration owns GPU handles.
...
bool FrameTransport::handleAcquireLine(int clientFd, const std::string& line) {
    const long long eye = parseKey(line, "eye", -1);
    const long long index = parseKey(line, "index", -1);
...
// Release a registered image. Caller holds eyesMutex_.
void FrameTransport::releaseSlotLocked(int eye, int imageIndex) {
...
void FrameTransport::storeEyeDmabuf(int eye, const EyeFrame& incoming) {
    std::lock_guard<std::mutex> lock(eyesMutex_);
    if (latest_[eye].kind != BufferKind::None &&
    ...
    buffers_[eye][incoming.imageIndex] = incoming; // Takes ownership of plane fds.
    buffers_[eye][incoming.imageIndex].registrationSerial = nextSerial_++;
}

void FrameTransport::releaseEye(int eye) {
    std::lock_guard<std::mutex> lock(eyesMutex_);
    if (latest_[eye].acquireFenceFd >= 0) {
        ::close(latest_[eye].acquireFenceFd);
    }
...
EyeFrame FrameTransport::pollEye(int eye) {
    if (eye < 0 || eye >= kEyeCount) return EyeFrame{};
    std::lock_guard<std::mutex> lock(eyesMutex_);
    EyeFrame snapshot = latest_[eye];
    latest_[eye].acquireFenceFd = -1; // Transfer fence ownership to the render thread.
    if (snapshot.kind != BufferKind::None) latestClaimed_[eye] = true;
    return snapshot;
}

void FrameTransport::publishReleaseFence(int eye, int imageIndex, int releaseFenceFd) {
...
void FrameTransport::discardFrame(int eye, int imageIndex, uint64_t serial) {
    if (eye < 0 || eye >= kEyeCount ||
...
bool FrameTransport::hasStereoContent() const {
